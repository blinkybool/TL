...
-- instance : has_zero term := ⟨term.var 0⟩

-- instance : has_one term := ⟨term.var 1⟩

-- def term_add : term → term → term
-- | (term.var n) (term.var m) := term.var (n+m)
-- | _ _ := star

-- instance : has_add term := ⟨term_add⟩
...
section substitution
  ...
  @[simp] lemma lift_var {n : ℕ} : ^n = n.succ := rfl
